...
serve(async (req) => {
  try {
    ...
    if (!action || !esp32_id || !esp32_timestamp || !idempotency_key) {
      return new Response(JSON.stringify({ error: "Missing required base fields" }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (Math.abs(nowTime - eventTime) > 5 * 60000) {
      return new Response(JSON.stringify({ error: "Timestamp too old or too far in future" }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (action === "validate_otp") {
      const { otp } = payload;
      if (!otp) return new Response(JSON.stringify({ error: "Missing OTP" }), { status: 400, headers: { "Content-Type": "application/json" } });

      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest("SHA-256", encoder.encode(otp));
      const otp_hash = Array.from(new Uint8Array(hashBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
      ...
      if (error || !otps || otps.length === 0) {
        await logHardwareEvent(supabaseAdmin, esp32_id, "authentication_failure", esp32_timestamp, idempotency_key);
      ...
      if ((record.locked_until && new Date(record.locked_until) > new Date()) || 
        ...
        await logHardwareEvent(supabaseAdmin, esp32_id, "authentication_failure", esp32_timestamp, idempotency_key, record.booking_id);
        return new Response(JSON.stringify({ success: false }), { status: 200, headers: { "Content-Type": "application/json" } });
      }

      // Success
      await supabaseAdmin.from("otps").update({ used_at: nowStr }).eq("id", record.id);
      await logHardwareEvent(supabaseAdmin, esp32_id, "authentication_success", esp32_timestamp, idempotency_key, record.booking_id);

      return new Response(JSON.stringify({ success: true, unlock_duration: 5000, booking_id: record.booking_id }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (action === "report_event") {
      ...
      if (!event_type) return new Response(JSON.stringify({ error: "Missing event_type" }), { status: 400, headers: { "Content-Type": "application/json" } });

      // 1. Check idempotency (insert event)
      const eventResult = await logHardwareEvent(supabaseAdmin, esp32_id, event_type, esp32_timestamp, idempotency_key, booking_id);
      ...
      if (eventResult.error && eventResult.error.code === '23505') {
        return new Response(JSON.stringify({ acknowledged: true, duplicate: true }), { status: 200, headers: { "Content-Type": "application/json" } });
      }
      ...
      if (event_type === "key_removed" && booking_id) {
        ...
        const { data: booking } = await supabaseAdmin.from("bookings").select("user_id").eq("id", booking_id).single();
        if (booking) {
      ...
      if (event_type === "key_returned" && booking_id) {
        ...
        await supabaseAdmin.from("key_sessions")
          .update({
...
async function logHardwareEvent(supabaseAdmin: any, esp32_id: string, event_type: string, esp32_timestamp: string, idempotency_key: string, booking_id?: string) {
